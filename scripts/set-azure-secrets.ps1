# PowerShell script to prompt for and set all required Azure deployment GitHub secrets
param(
    [string]$Repo = "JayPVentures-LLC/jayventures-labs"
)
...
foreach ($secret in $secrets) {
    ...
    if ($value -ne "") {
        ...
        gh secret set $($secret.Name) --repo $Repo --body "$value"
    } else {
        Write-Host "Skipped $($secret.Name)"
    }
}

Write-Host "All provided secrets set for $Repo."